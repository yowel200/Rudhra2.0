...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: IG_DESC }, async (message, match) => {
    ...
    if (type === 'image') { await message.sendMessage(data, MessageType.image, { caption: "```ᴍᴀᴅᴇ ʙʏ ᴘʀɪɴᴄᴇʀᴜᴅʜ```" }) }
    else if (type === 'video') { await message.sendMessage(data, MessageType.video, { caption: "```ᴍᴀᴅᴇ ʙʏ ᴘʀɪɴᴄᴇʀᴜᴅʜ```" }) }
});
//const axios = require('axios')
