...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅️Toxic Alexa working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 '));
          // thanks to afnanplk
...
*/
                        try {
                            ...
                            if (config.NOLOG == 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ ༄ALEXA࿐ ~______~*' +
                                    '\n\n*😵 ' + error + '*\n\n```Report errors in the group\njoin \nhttps://chat.whatsapp.com/KTcvOHqo3Kl1YxGpIdRjif``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
